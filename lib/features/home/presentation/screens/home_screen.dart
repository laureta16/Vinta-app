...
import 'package:provider/provider.dart';
import '../../../common/presentation/screens/vinta_notifications_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<ClothingItem> _applyCategory(List<ClothingItem> items) {
    switch (_selectedCategory) {
      case 'New In':
        return List<ClothingItem>.from(items)
          ..sort((a, b) => b.id.compareTo(a.id));
      case 'Trending':
        return List<ClothingItem>.from(items)
          ..sort((a, b) {
            final scoreA = (a.isLiked ? 2 : 0) + a.comments.length;
            final scoreB = (b.isLiked ? 2 : 0) + b.comments.length;
            return scoreB.compareTo(scoreA);
          });
      case 'Local':
        final auth = Provider.of<AuthProvider>(context, listen: false);
        final userCity = auth.user?.bio?.toLowerCase() ?? '';
        if (userCity.isEmpty) return items;
        return items
            .where((item) => item.city.toLowerCase().contains(userCity))
            .toList();
      case 'Bundles':
        return items.where((item) => item.price >= 5000).toList();
      case 'Discovery':
      default:
        return items;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final visibleItems = _applyCategory(sortedItems);
    final showSkeleton =
        clothingProvider.isLoading && clothingProvider.items.isEmpty;
    final showEmptyState = !clothingProvider.isLoading && visibleItems.isEmpty;

    return Scaffold(
      backgroundColor: AppColors.background,
      body: RefreshIndicator(
        onRefresh: clothingProvider.refreshItems,
        ...
        child: CustomScrollView(
          ...
          slivers: [
              ...
              flexibleSpace: FlexibleSpaceBar(
                ...
                title: Row(
                  ...
                  children: [
                    ...
                    const Spacer(),
                    _buildHeaderAction(Icons.search_rounded, onTap: () {
                    ...
                    Consumer<NotificationProvider>(
                      builder: (context, notif, _) => Badge(
                        ...
                        child: _buildHeaderAction(Icons.notifications_none_rounded, onTap: () {
                          Navigator.of(context).push(MaterialPageRoute(
                            builder: (_) => const VintaNotificationsScreen(),
                          ));
                        }),
                      ),
                    ),
            ...
            if (clothingProvider.errorMessage != null)
              SliverToBoxAdapter(
                child: Container(
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'No listings in $_selectedCategory right now',
              style: const TextStyle(
                color: AppColors.textPrimary,
            ...
            const Text(
              'Pull down to refresh or switch category.',
                  ...
                  child: ClipRRect(
                    ...
                    child: Image.network(
                      ...
                      loadingBuilder: (context, child, progress) =>
                          progress == null ? child : _buildShimmerPlaceholder(),
                    ),
                  ),
                ),
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(30)),
                    child: Text(
                        item.condition == ClothingCondition.brandNew
                            ? 'NEW'
                            : 'USED',
                        style: const TextStyle(
                            fontWeight: FontWeight.w900,
